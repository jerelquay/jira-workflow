...
/**
 * check for permission to move a release from Review to PendSIT state by building the binary artefacts
 */
try {
    //log.debug("start " + issue.key)
    def appIssue = JiraUtils.getAppIssue(issue)
    passesCondition = JiraUtils.checkCustomPermission(appIssue, CustomRelPermissions.BUILD)
} catch (Exception e) {
