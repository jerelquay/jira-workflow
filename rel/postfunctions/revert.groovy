 ...
 */
try {
    log.debug("Start revert" + issue.key)
    ...
    new Thread() {
        void run() {
            try {
                boolean isSuccess = JiraUtils.getDeployUtils(issue).undeploy(currentState, "revert")

                if (!isSuccess) {
                    log.info("Revert " + issue.getSummary() + "from " + currentState + " environment failed")
                }
                ...
                if (isSuccess) {
                    log.info("Revert successfully " + issue.getSummary() + "from " + currentState)
                }
            } catch (Exception e) {
                log.error("Revert script error", e)
                JiraUtils.updateMessageToUser(issue, "Revert error, please inform an administrator." + System.getProperty("line.separator") + e.toString())
            } finally {
